  ...
  export default {
    ...
    methods: {
      init() {
        ...
        this.layoutTree = nd3.tree().nodeSize([this.diamonds.intervalW, this.diamonds.intervalH]).separation(() => 1);
        // 主图
        this.svg = nd3.select('#penetrateChart').append('svg').attr('width', svgW).attr('height', svgH).attr('id', 'treesvg')
          .call(nd3.drag().on("drag", this.dragged).on("end", this.ended)); // .call(nd3.zoom().scaleExtent([0, 5]))
        this.svg.attr('style', 'position: relative;z-index: 2;')
        ...
        this.rootUp = nd3.hierarchy(upTree, d => d.children);
        this.rootUp.x0 = 0
        this.rootUp.y0 = 0
 
        this.rootDown = nd3.hierarchy(downTree, d => d.children);
        this.rootDown.x0 = 0
        this.rootDown.y0 = 0;
        // 上 和 下 结构
        let treeArr = [{
      ...
      dragged(d) {
        console.log("d", d);
        this.svg = this.svg.classed("dragging", true);
        this.svg.raise().attr("cx", d.subject.x = d.x).attr("cy", d.subject.y = d.y);
      },
      ended() {
